...
while True:
    print('''El programa debe ser colocado dentro de una carpeta, con el archivo .txt que se desea analizar Si ya cumples con estas condiciones, escribe:s''')
    start=input()
    if start.lower()=='s':
        break
load='|          |'
...
if Files=='':
    while True:
        print('No hay archivos .txt en la carpeta actual, desea analizar otra caperta, introduzca la dirección, si desea salir presione enter')
        p=input()
        if p.lower=='':
            sys.exit()
...
for txt in Files_txt:
    ...
    task=open(path,encoding='UTF-8') # actualmente no es necesario def el modo r ya que este se activa por def cada vez que llama a open
    emails=task.read()
...
if len(Match)>0:
    ...
    print('No se ha encontrado ningun correo en la carpeta contenedora, presione cualquier tecla para salir')
    s=input()
    if s=='':
        sys.exit()
    else:
